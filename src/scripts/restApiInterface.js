...
const post = async ({ hostUrl, apiKey, restPath, query, body }) => await request
  .get(`${hostUrl}/${restPath}`)
  .set("x-api-key", apiKey)
  .query(query)
  .send(body)
...
const put = async ({ hostUrl, apiKey, restPath, query, body }) => await request
...
const patch = async ({ hostUrl, apiKey, restPath, query, body }) => await request
  .patch(`${hostUrl}/${restPath}`)
  .set("x-api-key", apiKey)
  .query(query)
  .send(body)
  .then(({ statusCode, body }) => ({ statusCode, body }))
...
const apiInterface = (hostUrl, apiKey) => ({
  ...
  patch: async (params) => await patch({ hostUrl, apiKey, ...params }),
